...
md`# iPhone充电数据 -- 航嘉20W + 华为3A线`
)}

function _chargingPlot(Plot,plotData,tip50,tip90,tip95,tipVoltageChange,plotBattery){return(
Plot.plot({
  width: 1200,
  height: 660,
  marginLeft: 40,
  ...
  marginBottom: 30,
  marginTop: 20,
  x: { axis: null, transform: (d) => d/60, domain: [0,75] },
  y: { axis: null },
  marks: [
 ...
 //   Plot.tip([tip100.tipText], {x: tip100.duration, anchor: "top", dy: -110, textPadding: 4, strokeOpacity: 0.3}),
    ...
    () => Plot.plot({
      marks: [
        ...
        x: { axis: "top", transform: (d) => d/60, domain: [0,75], line: true, label: "时间（分钟）", labelOffset: -5, labelArrow: false },
        y: { domain: [26,42], ticks: 6, tickSize: 5, inset: true, line: true, nice: true, label: "电池温度（℃）", labelAnchor: "center", labelArrow: false },
      width: 1200,
      height: 200,
      marginLeft: 40,
      marginRight: 50,
    ...
    () => Plot.plot({
      ...
      x: { transform: (d) => d/60, domain: [0,75], label: "时间（分钟）", labelArrow: false },
...
{
  const url = "https://gist.githubusercontent.com/wn2try/47f45560b768da09f0d0061580450af7/raw/21a8887152307825be3c338b30090e8956ace7e2/231129_174439.csv";
  return fetch(url)
    .then((res) => res.text())
...
Plot.plot({
  ...
  marks: [
    Plot.dot(rawData, { filter: (d) => d.power > 0, x: "current", y: "voltage", r: 2, symbol: "circle", stroke: "darkcyan", strokeWidth: 0.8, tip: {anchor: "top-right", textPadding: 4, strokeOpacity: 0.1, fill: "darkcyan", fillOpacity: 0.1}, title: (d) => `航嘉20W ○\n电压：${d.voltage}V\n电流：${d.current}A`}),
    ...
    Plot.dotY(["航嘉20W"], {x: 2.4, y: 4.2, symbol: "circle", stroke: "darkcyan"}),
    Plot.text(["  航嘉20W 1C + 华为3A线"], {x: 2.4, y: 4.2, textAnchor: "start"}),
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("chargingPlot")).define("chargingPlot", ["Plot","plotData","tip50","tip90","tip95","tipVoltageChange","plotBattery"], _chargingPlot);
